...
Public CopyHistory() As History
Public CopyHistoryCount As Integer
Public CopyHistoryCurrent As Integer
...
Public EmptyMapTile As MapBlock
...
For X = .Pos.Left To .Pos.Right
    For Y = .Pos.Top To .Pos.Bottom
        MapData(CurrentMap).Tile(X, Y) = .Data(X, Y)
    Next Y
Next X
...
If InMapBounds(X, Y) And InMapBounds(X2, Y2) Then
...
Else
    For XX = X To X2
        For YY = Y To Y2
            If MapData(CurrentMap).Tile(XX, YY).Blocked <> Editado.Data(XX, YY).Blocked Or _
               MapData(CurrentMap).Tile(XX, YY).TileExit.X <> Editado.Data(XX, YY).TileExit.X Or _
               MapData(CurrentMap).Tile(XX, YY).CharIndex <> Editado.Data(XX, YY).CharIndex Or _
               MapData(CurrentMap).Tile(XX, YY).OBJInfo.OBJIndex <> Editado.Data(XX, YY).OBJInfo.OBJIndex Or _
               MapData(CurrentMap).Tile(XX, YY).Graphic(1).grhindex <> Editado.Data(XX, YY).Graphic(1).grhindex Or _
               MapData(CurrentMap).Tile(XX, YY).Graphic(2).grhindex <> Editado.Data(XX, YY).Graphic(2).grhindex Or _
               MapData(CurrentMap).Tile(XX, YY).Graphic(3).grhindex <> Editado.Data(XX, YY).Graphic(3).grhindex Or _
               MapData(CurrentMap).Tile(XX, YY).Graphic(4).grhindex <> Editado.Data(XX, YY).Graphic(4).grhindex Then
                Distinto = True
            End If
        Next YY
    Next XX
...
With Retroceder(NumRetroceder)
    ...
    For XX = X To X2
        For YY = Y To Y2
            .Data(XX, YY) = MapData(CurrentMap).Tile(XX, YY)
        Next YY
    Next XX
End With
End If
    ...
    For k = 1 To Cuantos
        ...
        If frmConfigSup.MOSAICO.value = vbChecked Then
          ...
          If frmMain.mnuAutoCompletarSuperficies.Checked = False Then
                ...
                End If
                MapData(CurrentMap).Tile(X, Y).Graphic(Val(frmMain.cCapas.text)).grhindex = aux
                InitGrh MapData(CurrentMap).Tile(X, Y).Graphic(Val(frmMain.cCapas.text)), aux
          Else
                ...
                For i = 1 To frmConfigSup.mLargo.text
                    For j = 1 To frmConfigSup.mAncho.text
                        ...
                        If frmMain.cInsertarBloqueo.value = True Then
                            MapData(CurrentMap).Tile(tXX, tYY).Blocked = 1
                        Else
                            MapData(CurrentMap).Tile(tXX, tYY).Blocked = 0
                        End If

                         MapData(CurrentMap).Tile(tXX, tYY).Graphic(Val(frmMain.cCapas.text)).grhindex = aux
                         
                         InitGrh MapData(CurrentMap).Tile(tXX, tYY).Graphic(Val(frmMain.cCapas.text)), aux
                         tXX = tXX + 1
                         desptile = desptile + 1
                    Next
                    tXX = X
    ...
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        ...
        If X < MinXBorder Or X > MaxXBorder Or Y < MinYBorder Or Y > MaxYBorder Then
          ...
          Else
            ...
            MapData(CurrentMap).Tile(X, Y).Graphic(Val(frmMain.cCapas.text)).grhindex = Val(frmMain.cGrh.text)
            
            'Setup GRH
    
            InitGrh MapData(CurrentMap).Tile(X, Y).Graphic(Val(frmMain.cCapas.text)), Val(frmMain.cGrh.text)
        End If
             'Erase NPCs
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        MapData(CurrentMap).Tile(X, Y).Blocked = Valor
    Next X
Next Y
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        If MapData(CurrentMap).Tile(X, Y).NPCIndex > 0 Then
            If (Hostiles = True And MapData(CurrentMap).Tile(X, Y).NPCIndex >= 500) Or (Hostiles = False And MapData(CurrentMap).Tile(X, Y).NPCIndex < 500) Then
                Call EraseChar(MapData(CurrentMap).Tile(X, Y).CharIndex)
                MapData(CurrentMap).Tile(X, Y).NPCIndex = 0
            End If
        End If
    Next X
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        If MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex > 0 Then
            If MapData(CurrentMap).Tile(X, Y).Graphic(3).grhindex = MapData(CurrentMap).Tile(X, Y).ObjGrh.grhindex Then MapData(CurrentMap).Tile(X, Y).Graphic(3).grhindex = 0
            MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex = 0
            MapData(CurrentMap).Tile(X, Y).OBJInfo.Amount = 0
        End If
    Next X
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        If MapData(CurrentMap).Tile(X, Y).Trigger > 0 Then
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        If Capa = 1 Then
            MapData(CurrentMap).Tile(X, Y).Graphic(Capa).grhindex = 1
        Else
            MapData(CurrentMap).Tile(X, Y).Graphic(Capa).grhindex = 0
        End If
    Next X
Next Y
...
tTrans = MapData(CurrentMap).Tile(tx, ty).TileExit
If tTrans.map > 0 Then

    If MapInfo(tTrans.map).Width = 0 Then
        Call frmMain.OpenMap(tTrans.map)
    End If
        
        If UserMap <> tTrans.map Then
            frmMain.picRadar.Cls
            If FileExist(DirMaps & "\Mini" & tTrans.map & ".bmp", vbArchive) Then
                frmMain.picRadar.Picture = LoadPicture(DirMaps & "\Mini" & tTrans.map & ".bmp")
            End If
        End If
...
End If
    ...
    If Button = vbRightButton Then
        ...
        ' Bloqueos
        If MapData(CurrentMap).Tile(tx, ty).Blocked = 1 Then frmMain.StatTxt.text = frmMain.StatTxt.text & " (BLOQ)"
        
        ' Translados
        If MapData(CurrentMap).Tile(tx, ty).TileExit.map > 0 Then
            If frmMain.mnuAutoCapturarTranslados.Checked = True Then
                frmMain.tTMapa.text = MapData(CurrentMap).Tile(tx, ty).TileExit.map
                frmMain.tTX.text = MapData(CurrentMap).Tile(tx, ty).TileExit.X
                frmMain.tTY = MapData(CurrentMap).Tile(tx, ty).TileExit.Y
            End If
            frmMain.StatTxt.text = frmMain.StatTxt.text & " (Trans.: " & MapData(CurrentMap).Tile(tx, ty).TileExit.map & "," & MapData(CurrentMap).Tile(tx, ty).TileExit.X & "," & MapData(CurrentMap).Tile(tx, ty).TileExit.Y & ")"
        End If
        
        ' NPCs
        If MapData(CurrentMap).Tile(tx, ty).NPCIndex > 0 Then
            If MapData(CurrentMap).Tile(tx, ty).NPCIndex > 499 Then
                frmMain.StatTxt.text = frmMain.StatTxt.text & " (NPC-Hostil: " & MapData(CurrentMap).Tile(tx, ty).NPCIndex & " - " & NpcData(MapData(CurrentMap).Tile(tx, ty).NPCIndex).name & ")"
            Else
                frmMain.StatTxt.text = frmMain.StatTxt.text & " (NPC: " & MapData(CurrentMap).Tile(tx, ty).NPCIndex & " - " & NpcData(MapData(CurrentMap).Tile(tx, ty).NPCIndex).name & ")"
            End If
        End If
        
        ' OBJs
        If MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex > 0 Then
            frmMain.StatTxt.text = frmMain.StatTxt.text & " (Obj: " & MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex & " - " & ObjData(MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex).name & " - Cant.:" & MapData(CurrentMap).Tile(tx, ty).OBJInfo.Amount & ")"
        End If
        
        ' Capas
        frmMain.StatTxt.text = frmMain.StatTxt.text & ENDL & "Capa1: " & MapData(CurrentMap).Tile(tx, ty).Graphic(1).grhindex & " - Capa2: " & MapData(CurrentMap).Tile(tx, ty).Graphic(2).grhindex & " - Capa3: " & MapData(CurrentMap).Tile(tx, ty).Graphic(3).grhindex & " - Capa4: " & MapData(CurrentMap).Tile(tx, ty).Graphic(4).grhindex
        If frmMain.mnuAutoCapturarSuperficie.Checked = True And frmMain.cSeleccionarSuperficie.value = False Then
            If MapData(CurrentMap).Tile(tx, ty).Graphic(4).grhindex <> 0 Then
                frmMain.cCapas.text = 4
                frmMain.cGrh.text = MapData(CurrentMap).Tile(tx, ty).Graphic(4).grhindex
            ElseIf MapData(CurrentMap).Tile(tx, ty).Graphic(3).grhindex <> 0 Then
                frmMain.cCapas.text = 3
                frmMain.cGrh.text = MapData(CurrentMap).Tile(tx, ty).Graphic(3).grhindex
            ElseIf MapData(CurrentMap).Tile(tx, ty).Graphic(2).grhindex <> 0 Then
                frmMain.cCapas.text = 2
                frmMain.cGrh.text = MapData(CurrentMap).Tile(tx, ty).Graphic(2).grhindex
            ElseIf MapData(CurrentMap).Tile(tx, ty).Graphic(1).grhindex <> 0 Then
                frmMain.cCapas.text = 1
                frmMain.cGrh.text = MapData(CurrentMap).Tile(tx, ty).Graphic(1).grhindex
            End If
        End If
    ...
    If Button = vbLeftButton Then
        ...
        ElseIf CopyState = 4 Then
            If (mCopyX <> tx Or mCopyY <> ty) And (CopyRect.Left > 0 And CopyRect.Right > 0 And CopyRect.Top > 0 And CopyRect.Bottom > 0) Then
            mCopyX = tx
            mCopyY = ty
            Call AddDeshacer(tx - AddMX, ty - AddMY, CopyRect.Right - CopyRect.Left + tx - AddMX, CopyRect.Bottom - CopyRect.Top + ty - AddMY)
            If frmMain.Cortado Then 'Si corto, en el espacio que corto tiene que quedar todo vacio
                frmMain.Cortado = False
                Call AddDeshacer(CopyRect.Left, CopyRect.Top, CopyRect.Right, CopyRect.Bottom)
                For X = CopyRect.Left To CopyRect.Right
                    For Y = CopyRect.Top To CopyRect.Bottom
                        MapData(CurrentMap).Tile(X, Y) = EmptyMapTile
                    Next Y
                Next X
                Call AddEditado(CopyRect.Left, CopyRect.Top, CopyRect.Right, CopyRect.Bottom)
            End If
            
            For X = CopyRect.Left To CopyRect.Right
                For Y = CopyRect.Top To CopyRect.Bottom
                    'MapCopyD(X - CopyRect.Left, Y - CopyRect.Top) = MapData(CurrentMap).Tile(tx + X - CopyRect.Left - AddMX, ty + Y - CopyRect.Top - AddMY)
                    If InMapBounds(tx + X - CopyRect.Left - AddMX, ty + Y - CopyRect.Top - AddMY) Then
                        MapData(CurrentMap).Tile(tx + X - CopyRect.Left - AddMX, ty + Y - CopyRect.Top - AddMY) = MapCopy(X - CopyRect.Left, Y - CopyRect.Top)
                    End If
                Next Y
            Next X
        ...
        For Y = PRect.Top + 1 To PRect.Bottom
            ...
            If frmMain.cQuitarEnTodasLasCapas.value = True Then
                ...
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                If MapData(CurrentMap).Tile(tx, ty).LightIndex > 0 Then
                    Call Light_Destroy(MapData(CurrentMap).Tile(tx, ty).LightIndex)
                End If
                
                For LoopC = 2 To 3
                    MapData(CurrentMap).Tile(tx, ty).Graphic(LoopC).grhindex = 0
                Next LoopC
            ...
            If frmMain.cQuitarEnEstaCapa.value = True Then
                If Val(frmMain.cCapas.text) = 1 Then
                    If MapData(CurrentMap).Tile(tx, ty).Graphic(1).grhindex <> 1 Then
                        MapInfo(CurrentMap).Changed = 1 'Set changed flag
                        MapData(CurrentMap).Tile(tx, ty).Graphic(1).grhindex = 1
                        'Exit Sub
                    End If
                ElseIf MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)).grhindex <> 0 Then
                    MapInfo(CurrentMap).Changed = 1 'Set changed flag
                    ...
                    MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)).grhindex = 0
                    'Exit Sub
                End If
            End If
        If PonerLuz > 0 Then
            If MapData(CurrentMap).Tile(tx, ty).LightIndex > 0 Then
                Call Light_Destroy(MapData(CurrentMap).Tile(tx, ty).LightIndex)
            End If
            MapData(CurrentMap).Tile(tx, ty).LightIndex = Light_Create(tx, ty, 255, 255, 255, Val(frmMain.tRango.text), PonerLuz - 1)
            MapData(CurrentMap).Tile(tx, ty).LuzRango = Val(frmMain.tRango.text)
            MapData(CurrentMap).Tile(tx, ty).Graphic(3).grhindex = -PonerLuz
        End If
        '************** Place grh
        If frmMain.cSeleccionarSuperficie.value = True Then
            ...
            If frmConfigSup.MOSAICO.value = vbChecked Then
              ...
              If frmMain.mnuAutoCompletarSuperficies.Checked = False Then
                    ...
                    (((ty + dy) Mod frmConfigSup.mLargo.text) * frmConfigSup.mAncho.text) + ((tx + dX) Mod frmConfigSup.mAncho.text)
                     If MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)).grhindex <> aux Or MapData(CurrentMap).Tile(tx, ty).Blocked <> frmMain.SelectPanel(2).value Then
                        MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)).grhindex = aux
                        InitGrh MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)), aux
                    End If
              Else
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                Dim tXX As Integer, tYY As Integer, i As Integer, j As Integer, desptile As Integer
            ...
            Else
                'Else Place graphic
                If MapData(CurrentMap).Tile(tx, ty).Blocked <> frmMain.SelectPanel(2).value Or MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)).grhindex <> Val(frmMain.cGrh.text) Then
                    MapInfo(CurrentMap).Changed = 1 'Set changed flag
                    MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)).grhindex = Val(frmMain.cGrh.text)
                    'Setup GRH
                    InitGrh MapData(CurrentMap).Tile(tx, ty).Graphic(Val(frmMain.cCapas.text)), Val(frmMain.cGrh.text)
                End If
            End If
        ...
        '************** Place blocked tile
        If frmMain.cInsertarBloqueo.value = True Then
            If MapData(CurrentMap).Tile(tx, ty).Blocked <> 1 Then

                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                MapData(CurrentMap).Tile(tx, ty).Blocked = 1
            End If
        ElseIf frmMain.cQuitarBloqueo.value = True Then
            If MapData(CurrentMap).Tile(tx, ty).Blocked <> 0 Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                MapData(CurrentMap).Tile(tx, ty).Blocked = 0
            End If
        End If
        ...
        If frmMain.cInsertarTrans.value = True Then
            If Cfg_TrOBJ > 0 And Cfg_TrOBJ <= NumOBJs And frmMain.cInsertarTransOBJ.value = True Then
                If ObjData(Cfg_TrOBJ).OBJType = 19 Then
                    MapInfo(CurrentMap).Changed = 1 'Set changed flag
                    InitGrh MapData(CurrentMap).Tile(tx, ty).ObjGrh, ObjData(Cfg_TrOBJ).grhindex
                    MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex = Cfg_TrOBJ
                    MapData(CurrentMap).Tile(tx, ty).OBJInfo.Amount = 1
                End If
            End If
            ...
            End If
                    ...
                    MapInfo(CurrentMap).Changed = 1 'Set changed flag
                    MapData(CurrentMap).Tile(tx, ty).TileExit.map = Val(frmMain.tTMapa.text)
                    MapData(CurrentMap).Tile(tx, ty).TileExit.X = Val(frmMain.tTX.text)
                    MapData(CurrentMap).Tile(tx, ty).TileExit.Y = Val(frmMain.tTY.text)
             
        ElseIf frmMain.cQuitarTrans.value = True Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                MapData(CurrentMap).Tile(tx, ty).TileExit.map = 0
                MapData(CurrentMap).Tile(tx, ty).TileExit.X = 0
                MapData(CurrentMap).Tile(tx, ty).TileExit.Y = 0
        End If
        ...
        If frmMain.cInsertarFunc(0).value = True Then
            If frmMain.cNumFunc(0).text > 0 Then
                ...
                If NPCIndex <> MapData(CurrentMap).Tile(tx, ty).NPCIndex Then
                    MapInfo(CurrentMap).Changed = 1 'Set changed flag
                    Body = NpcData(NPCIndex).Body
                    Head = NpcData(NPCIndex).Head
                    Heading = NpcData(NPCIndex).Heading
                    Call MakeChar(NextOpenChar(), Body, Head, Heading, tx, ty)
                    MapData(CurrentMap).Tile(tx, ty).NPCIndex = NPCIndex
                End If
            End If
        ...
        ElseIf frmMain.cQuitarFunc(0).value = True Or frmMain.cQuitarFunc(1).value = True Then
            If MapData(CurrentMap).Tile(tx, ty).NPCIndex > 0 Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                MapData(CurrentMap).Tile(tx, ty).NPCIndex = 0
                Call EraseChar(MapData(CurrentMap).Tile(tx, ty).CharIndex)
            End If
        End If
        ...
        If frmMain.cInsertarFunc(2).value = True Then ' Insertar Objeto
            If frmMain.cNumFunc(2).text > 0 Then
                ...
                If MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex <> OBJIndex Or MapData(CurrentMap).Tile(tx, ty).OBJInfo.Amount <> Val(frmMain.cCantFunc(2).text) Then
                    MapInfo(CurrentMap).Changed = 1 'Set changed flag
                    InitGrh MapData(CurrentMap).Tile(tx, ty).ObjGrh, ObjData(OBJIndex).grhindex
                    MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex = OBJIndex
                    MapData(CurrentMap).Tile(tx, ty).OBJInfo.Amount = Val(frmMain.cCantFunc(2).text)
                    Select Case ObjData(OBJIndex).OBJType
                        Case 4, 8, 10, 22 ' Arboles, Carteles, Foros, Yacimientos
                            MapData(CurrentMap).Tile(tx, ty).Graphic(3) = MapData(CurrentMap).Tile(tx, ty).ObjGrh
                    End Select
                End If
            End If
        ElseIf frmMain.cQuitarFunc(2).value = True Then ' Quitar Objeto
            If MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex <> 0 Or MapData(CurrentMap).Tile(tx, ty).OBJInfo.Amount <> 0 Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                If MapData(CurrentMap).Tile(tx, ty).Graphic(3).grhindex = MapData(CurrentMap).Tile(tx, ty).ObjGrh.grhindex Then MapData(CurrentMap).Tile(tx, ty).Graphic(3).grhindex = 0
                MapData(CurrentMap).Tile(tx, ty).ObjGrh.grhindex = 0
                MapData(CurrentMap).Tile(tx, ty).OBJInfo.OBJIndex = 0
                MapData(CurrentMap).Tile(tx, ty).OBJInfo.Amount = 0
            End If
        End If
        
        ' ***************** Control de Funcion de Triggers *****************
        If frmMain.cInsertarTrigger.value = True Then ' Insertar Trigger
            If MapData(CurrentMap).Tile(tx, ty).Trigger <> frmMain.lListado(4).ListIndex Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                MapData(CurrentMap).Tile(tx, ty).Trigger = frmMain.lListado(4).ListIndex
            End If
        ElseIf frmMain.cQuitarTrigger.value = True Then ' Quitar Trigger
            If MapData(CurrentMap).Tile(tx, ty).Trigger <> 0 Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                MapData(CurrentMap).Tile(tx, ty).Trigger = 0
            End If
        End If
        ...
        ElseIf frmParticle.cmdDel.value = True Then ' Quitar Particle
            If MapData(CurrentMap).Tile(tx, ty).particle_group_index <> 0 Then
                MapInfo(CurrentMap).Changed = 1 'Set changed flag
                engine.Particle_Group_Remove MapData(CurrentMap).Tile(tx, ty).particle_group_index
                MapData(CurrentMap).Tile(tx, ty).particle_group_index = 0
            End If
        End If
        Next Y
        Next X
