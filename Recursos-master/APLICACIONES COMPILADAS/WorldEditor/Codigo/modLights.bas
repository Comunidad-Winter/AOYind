...
Public Function Light_Create(ByVal MAP_X As Integer, ByVal MAP_Y As Integer, ByVal red As Byte, _
                        ...
                        Optional ByVal RANGE As Byte = 1, Optional ByVal Direccion As Byte = 0, Optional ByVal ID As Long) As Long
'**************************************************************
'Author: Aaron Perkins
...
'   OPTIMIZAR SUB CUANDO SE PUEDA!!!!!
    ...
    For i = 1 To light_count
        If light_list(i).MAP_X > UserPos.X - 22 And light_list(i).MAP_X < UserPos.X + 22 Then
            If light_list(i).MAP_Y > UserPos.Y - 20 And light_list(i).MAP_Y < UserPos.Y + 20 Then
        ...
        max_y = light_list(light_index).MAP_Y + light_list(light_index).RANGE * IIf(light_list(light_index).Direccion And 8, 0, 1)
            ...
            For Xa = min_x To max_x
                If InMapBounds(Xa, Ya) Then
                    XCoord = Xa
                    YCoord = Ya
                    MapData(CurrentMap).Tile(Xa, Ya).light_value(0) = CalcularRadio(light_list(light_index).RANGE, _
                    light_list(light_index).MAP_X, light_list(light_index).MAP_Y, XCoord, _
                    YCoord, MapData(CurrentMap).Tile(Xa, Ya).light_value(0), LightColor, IluRGB)

                    XCoord = Xa + 1
                    YCoord = Ya
                    MapData(CurrentMap).Tile(Xa, Ya).light_value(1) = CalcularRadio(light_list(light_index).RANGE, _
                    light_list(light_index).MAP_X, light_list(light_index).MAP_Y, XCoord, _
                    YCoord, MapData(CurrentMap).Tile(Xa, Ya).light_value(1), LightColor, IluRGB)
                       
                    XCoord = Xa
                    YCoord = Ya + 1
                    MapData(CurrentMap).Tile(Xa, Ya).light_value(2) = CalcularRadio(light_list(light_index).RANGE, _
                    light_list(light_index).MAP_X, light_list(light_index).MAP_Y, XCoord, _
                    YCoord, MapData(CurrentMap).Tile(Xa, Ya).light_value(2), LightColor, IluRGB)
                    
                    XCoord = Xa + 1
                    YCoord = Ya + 1
                    MapData(CurrentMap).Tile(Xa, Ya).light_value(3) = CalcularRadio(light_list(light_index).RANGE, _
                    light_list(light_index).MAP_X, light_list(light_index).MAP_Y, XCoord, _
                    YCoord, MapData(CurrentMap).Tile(Xa, Ya).light_value(3), LightColor, IluRGB)
                End If
            Next Xa
...
Public Sub Light_Destroy_ToMap(ByVal X As Byte, ByVal Y As Byte)
    ...
    Next Index
End Sub
Public Sub Light_Destroy(ByVal light_index As Long)
'**************************************************************
'Author: Aaron Perkins
'Last Modify Date: 10/07/2002
'
...
Public Sub Light_Erase(ByVal light_index As Long)
'***************************************'
'Author: Juan Martín Sotuyo Dodero
...
'***************************************'
    ...
    If InMapBounds(min_x, min_y) Then
        MapData(CurrentMap).Tile(min_x, min_y).light_value(2) = colorz
    End If
    'NW
    If InMapBounds(max_x, min_y) Then
        MapData(CurrentMap).Tile(max_x, min_y).light_value(0) = colorz
    End If
    'SW
    If InMapBounds(max_x, max_y) Then
        MapData(CurrentMap).Tile(max_x, max_y).light_value(1) = colorz
    End If
    'SE
    If InMapBounds(min_x, max_y) Then
        MapData(CurrentMap).Tile(min_x, max_y).light_value(3) = colorz
    End If
    ...
    For X = min_x + 1 To max_x - 1
        If InMapBounds(X, min_y) Then
            MapData(CurrentMap).Tile(X, min_y).light_value(0) = colorz
            MapData(CurrentMap).Tile(X, min_y).light_value(2) = colorz
        End If
    Next X
    ...
    For X = min_x + 1 To max_x - 1
        If InMapBounds(X, max_y) Then
            MapData(CurrentMap).Tile(X, max_y).light_value(1) = colorz
            MapData(CurrentMap).Tile(X, max_y).light_value(3) = colorz
        End If
    Next X
    ...
    For Y = min_y + 1 To max_y - 1
        If InMapBounds(min_x, Y) Then
            MapData(CurrentMap).Tile(min_x, Y).light_value(2) = colorz
            MapData(CurrentMap).Tile(min_x, Y).light_value(3) = colorz
        End If
    Next Y
    ...
    For Y = min_y + 1 To max_y - 1
        If InMapBounds(max_x, Y) Then
            MapData(CurrentMap).Tile(max_x, Y).light_value(0) = colorz
            MapData(CurrentMap).Tile(max_x, Y).light_value(1) = colorz
        End If
    Next Y
    ...
    For X = min_x + 1 To max_x - 1
        For Y = min_y + 1 To max_y - 1
            If InMapBounds(X, Y) Then
                MapData(CurrentMap).Tile(X, Y).light_value(0) = colorz
                MapData(CurrentMap).Tile(X, Y).light_value(1) = colorz
                MapData(CurrentMap).Tile(X, Y).light_value(2) = colorz
                MapData(CurrentMap).Tile(X, Y).light_value(3) = colorz
            End If
        Next Y
    Next X
