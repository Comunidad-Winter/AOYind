...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        ...
        If MapData(CurrentMap).Tile(X, Y).Blocked = 1 Then
            If MapData(CurrentMap).Tile(X, Y).TileExit.Map > 0 And chkQuitarTrans.value = 1 Then ' Quita Translado Bloqueado
                MapData(CurrentMap).Tile(X, Y).TileExit.Map = 0
                MapData(CurrentMap).Tile(X, Y).TileExit.Y = 0
                MapData(CurrentMap).Tile(X, Y).TileExit.X = 0
            ElseIf MapData(CurrentMap).Tile(X, Y).Trigger > 0 And chkQuitarTrigBloq.value = 1 Then ' Quita Trigger Bloqueado
                MapData(CurrentMap).Tile(X, Y).Trigger = 0
            End If
        End If
        ' ** Quitar Triggers en Translado
        If MapData(CurrentMap).Tile(X, Y).TileExit.Map > 0 And chkQuitarTrigTrans.value = 1 Then
            If MapData(CurrentMap).Tile(X, Y).Trigger > 0 Then ' Quita Trigger en Translado
                MapData(CurrentMap).Tile(X, Y).Trigger = 0
            End If
        End If
        ' ** Mapea Arboles, Carteles, Foros y Yacimientos que no esten en la 3ra Capa
        If MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex > 0 And (chkMapearArbolesEtc.value = 1 Or chkBloquearArbolesEtc.value = 1) Then
            Select Case ObjData(MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex).OBJType
                Case 4, 8, 10, 22 ' Arboles, Carteles, Foros, Yacimientos
                    If MapData(CurrentMap).Tile(X, Y).Graphic(3).grhindex <> MapData(CurrentMap).Tile(X, Y).ObjGrh.grhindex And chkMapearArbolesEtc.value = 1 Then MapData(CurrentMap).Tile(X, Y).Graphic(3) = MapData(CurrentMap).Tile(X, Y).ObjGrh
                    If chkBloquearArbolesEtc.value = 1 And MapData(CurrentMap).Tile(X, Y).Blocked = 0 Then MapData(CurrentMap).Tile(X, Y).Blocked = 1
            End Select
        End If
        ' ** Mapea Arboles, Carteles, Foros y Yacimientos que no esten en la 3ra Capa
    Next X
