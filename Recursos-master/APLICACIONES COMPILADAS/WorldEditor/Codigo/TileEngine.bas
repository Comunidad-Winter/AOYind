...
Public GrabarParcial As Boolean
Public EditLimit As RECT
'Encabezado bmp
...
'******************************************
    ...
    ty = UserPos.Y + viewPortY \ 32 - frmMain.Renderer.ScaleHeight \ 64 - 1
    Debug.Print tx; ty
End Sub
...
#End If
    ...
    End With
End Sub
Sub MakeChar(ByVal CharIndex As Integer, ByVal Body As Integer, ByVal Head As Integer, ByVal Heading As Byte, ByVal X As Integer, ByVal Y As Integer, Optional Map As Integer = 0)
On Error Resume Next

If Map = 0 Then Map = CurrentMap
    ...
    'Plot on map
    MapData(Map).Tile(X, Y).CharIndex = CharIndex
End Sub
...
On Error Resume Next
    ...
    MapData(CurrentMap).Tile(charlist(CharIndex).Pos.X, charlist(CharIndex).Pos.Y).CharIndex = 0
    ...
    For j = UserPos.X - 8 To UserPos.X + 8
        For k = UserPos.Y - 6 To UserPos.Y + 6
            If InMapBounds(CLng(j), CLng(k)) Then
                If MapData(CurrentMap).Tile(j, k).ObjGrh.grhindex = GrhFogata Then
                    location.X = j
                    location.Y = k
...
'*************************************************************
    ...
    End If
        ...
        file_path = App.Path & "\graficos\" & GrhData(grh_index).FileNum & ".png"
