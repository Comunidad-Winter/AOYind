...
If GetKeyState(vbKeyUp) < 0 Or GetKeyState(vbKeyW) < 0 Then
    ...
    If GetKeyState(vbKeyRight) < 0 Or GetKeyState(vbKeyD) < 0 Then
        If UserPos.X > MapInfo(CurrentMap).Width + 32 Then Exit Sub ' 89
    ...
    If GetKeyState(vbKeyDown) < 0 Or GetKeyState(vbKeyS) < 0 Then
        If UserPos.Y > MapInfo(CurrentMap).Height + 26 Then Exit Sub ' 92
        If LegalPos(UserPos.X, UserPos.Y + 1) And WalkMode = True Then
            If dLastWalk + 50 > GetTickCount Then Exit Sub
            UserPos.Y = UserPos.Y + 1
            MoveCharbyPos UserCharIndex, UserPos.X, UserPos.Y
...
DirMaps = autoCompletaPath(Leer.GetValue("PATH", "DirMaps"))
If DirMaps = "\" Then
    DirMaps = IniPath & "Mapas\"
End If
If FileExist(DirMaps, vbDirectory) = False Then
    MsgBox "El directorio de Maps es incorrecto", vbCritical + vbOKOnly
    End
End If
...
If UserPos.X < XMinMapSize Or UserPos.X > MapInfo(CurrentMap).Width Then
    UserPos.X = 50
End If
If UserPos.Y < YMinMapSize Or UserPos.Y > MapInfo(CurrentMap).Height Then
    UserPos.Y = 50
End If
...
For i = 1 To MapInfo(CurrentMap).Width
    For e = 1 To MapInfo(CurrentMap).Height
        MapData(CurrentMap).Tile(i, e).NpcArea = 0
    Next e
Next i
...
Do While Not Encuentra And Itera < 500
    ...
    If Encuentra Then
        MapData(CurrentMap).Tile(X, Y).NpcArea = NPCIndex
    End If
Loop
End Sub
...
Dim UserIndex As Integer
    ...
    If AguaValida = 0 Then
        LegalPosNPC = (MapData(CurrentMap).Tile(X, Y).Blocked <> 1) And _
        (MapData(CurrentMap).Tile(X, Y).NpcArea = 0) And _
        (MapData(CurrentMap).Tile(X, Y).NPCIndex = 0) And _
        (MapData(CurrentMap).Tile(X, Y).Trigger <> 3) _
        And Not HayAgua(X, Y)
    Else
        LegalPosNPC = (MapData(CurrentMap).Tile(X, Y).Blocked <> 1) And _
        (MapData(CurrentMap).Tile(X, Y).NpcArea = 0) And _
        (MapData(CurrentMap).Tile(X, Y).NPCIndex = 0) And _
        (MapData(CurrentMap).Tile(X, Y).Trigger <> 3)
    End If
End Function
Function HayAgua(ByVal X As Integer, ByVal Y As Integer) As Boolean

If X > 0 And X <= MapInfo(CurrentMap).Width And Y > 0 And Y <= MapInfo(CurrentMap).Height Then
    If ((MapData(CurrentMap).Tile(X, Y).Graphic(1).grhindex >= 1505 And MapData(CurrentMap).Tile(X, Y).Graphic(1).grhindex <= 1520) Or _
    (MapData(CurrentMap).Tile(X, Y).Graphic(1).grhindex >= 5665 And MapData(CurrentMap).Tile(X, Y).Graphic(1).grhindex <= 5680) Or _
    (MapData(CurrentMap).Tile(X, Y).Graphic(1).grhindex >= 13547 And MapData(CurrentMap).Tile(X, Y).Graphic(1).grhindex <= 13562)) And _
       MapData(CurrentMap).Tile(X, Y).Graphic(2).grhindex = 0 Then
            HayAgua = True
    Else
            HayAgua = False
    End If
...
If WalkMode = False Then
    ...
    MapData(CurrentMap).Tile(UserPos.X, UserPos.Y).CharIndex = 0
Else
    'MakeCharacter
    If LegalPos(UserPos.X, UserPos.Y) Then
        Call MakeChar(NextOpenChar(), 1, 1, SOUTH, UserPos.X, UserPos.Y)
        UserCharIndex = MapData(CurrentMap).Tile(UserPos.X, UserPos.Y).CharIndex
...
If grhindex = 7284 Or grhindex = 7290 Or grhindex = 7291 Or grhindex = 7297 Or _
   ...
   grhindex = 7373 Or grhindex = 7375 Or grhindex = 7376 Then MapData(CurrentMap).Tile(X, Y).Graphic(2).grhindex = 0
...
'        MapData(CurrentMap).Tile(charlist(loopc).Pos.X, charlist(loopc).Pos.Y).CharIndex = loopc
