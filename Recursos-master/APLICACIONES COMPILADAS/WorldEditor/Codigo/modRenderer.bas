        ...
        If InMapBounds(X, Y) Then
            ...
            If SobreX = X And SobreY = Y Then
                ...
                If frmMain.cSeleccionarSuperficie.value = True Then
                    Sobre = MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex
                    If frmConfigSup.MOSAICO.value = vbChecked Then
                        ...
                        If frmMain.mnuAutoCompletarSuperficies.Checked = False Then
                            ...
                            If MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex <> aux Then
                                MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex = aux
                                InitGrh MapData(CurrentMap).Tile(X, Y).Graphic(bCapa), aux
                            End If
                        Else
                            aux = Val(frmMain.cGrh.text) + _
                            (((Y + dy) Mod frmConfigSup.mLargo.text) * frmConfigSup.mAncho.text) + ((X + dX) Mod frmConfigSup.mAncho.text)
                        ...
                        End If
                    Else
                        If MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex <> Val(frmMain.cGrh.text) Then
                            MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex = Val(frmMain.cGrh.text)
                            InitGrh MapData(CurrentMap).Tile(X, Y).Graphic(bCapa), Val(frmMain.cGrh.text)
                        End If
                    End If
                End If
            Else
                Sobre = -1
            End If
            With MapData(CurrentMap).Tile(X, Y).Graphic(1)
                If (.grhindex <> 0) Then
                    If (.Started = 1) Then
                        If (.Speed > 0) Then
                            .Speed = .Speed - 1
            ...
            End If
            'Layer 2 **********************************
            If MapData(CurrentMap).Tile(X, Y).Graphic(2).grhindex <> 0 And (frmMain.mnuVerCapa2.Checked = True) Then
                ''Call DDrawTransGrhtoSurface( _
                        BMPSurface, _
                        MapData(CurrentMap).Tile(X, y).Graphic(2), _
                        ((32 * ScreenX) - 32) + 0, _
                        ((32 * ScreenY) - 32) + 0, _
                        1, _
                        1)
            End If
            If Sobre >= 0 Then
                If MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex <> Sobre Then
                MapData(CurrentMap).Tile(X, Y).Graphic(bCapa).grhindex = Sobre
                InitGrh MapData(CurrentMap).Tile(X, Y).Graphic(bCapa), Sobre
                End If
            End If
        End If
        ScreenX = ScreenX + 1
    ...
    For X = 1 To 100
        If InMapBounds(X, Y) Then
            ...
            iPPy = ((32 * ScreenY) - 32) + 0
             'Object Layer **********************************
             If MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex <> 0 And frmMain.mnuVerObjetos.Checked = True Then
                 ''Call DDrawTransGrhtoSurface( _
                         BMPSurface, _
                         MapData(CurrentMap).Tile(X, y).ObjGrh, _
                         iPPx, iPPy, 1, 1)
             End If
            
                  'Char layer **********************************
                 If MapData(CurrentMap).Tile(X, Y).CharIndex <> 0 And frmMain.mnuVerNPCs.Checked = True Then
                 
                     TempChar = charlist(MapData(CurrentMap).Tile(X, Y).CharIndex)
                 ...
                 End If
             'Layer 3 *****************************************
             If MapData(CurrentMap).Tile(X, Y).Graphic(3).grhindex <> 0 And (frmMain.mnuVerCapa3.Checked = True) Then
                 'Draw
                 ''Call DDrawTransGrhtoSurface( _
                         BMPSurface, _
                         MapData(CurrentMap).Tile(X, y).Graphic(3), _
                         ((32 * ScreenX) - 32) + 0, _
                         ((32 * ScreenY) - 32) + 0, _
                         1, 1)
             End If
...
For Y = 1 To 100
    ...
    For X = 1 To 100
        If X < 101 And X > 0 And Y < 101 And Y > 0 Then ' 30/05/2006
            If MapData(CurrentMap).Tile(X, Y).Graphic(4).grhindex <> 0 _
            And (frmMain.mnuVerCapa4.Checked = True) Then
                'Draw
                'Call DDrawTransGrhtoSurface( _
                    BMPSurface, _
                    MapData(CurrentMap).Tile(X, y).Graphic(4), _
                    ((32 * ScreenX) - 32) + 0, _
                    ((32 * ScreenY) - 32) + 0, _
                    1, 1)
            End If
            If MapData(CurrentMap).Tile(X, Y).TileExit.Map <> 0 And frmMain.mnuVerTranslados.Checked = True Then
                Grh.grhindex = 3
                Grh.FrameCounter = 1
                Grh.Started = 0
                'Call DDrawTransGrhtoSurface( _
            ...
            End If
            'Show blocked tiles
            If frmMain.cVerBloqueos.value = True And MapData(CurrentMap).Tile(X, Y).Blocked = 1 Then
            ...
            If frmMain.cVerTriggers.value = True Then
                'Call DrawText(PixelPos(ScreenX), PixelPos(ScreenY), Str(MapData(CurrentMap).Tile(X, y).Trigger), vbRed)
            End If
        End If
        ScreenX = ScreenX + 1
    Next X
...
''''''''''''''''''''''''''''''''''''''''''''''''''''''''
If RenderToBMP Then
    SavePicture frmRenderer.Picture1.Picture, App.Path & "\" & MapInfo(CurrentMap).name & ".bmp"
Else
    Set c = New cDIBSection
    c.CreateFromPicture frmRenderer.Picture1.Picture
    Call SaveJPG(c, App.Path & "\" & MapInfo(CurrentMap).name & ".jpg")
    Set c = Nothing
End If
''''''''''''''''''''''''''''''''''''''''''''''''''''''''
