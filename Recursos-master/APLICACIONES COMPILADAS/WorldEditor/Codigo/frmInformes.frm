...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        If MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex > 0 Then
            txtInfo.text = txtInfo.text & vbCrLf & X & "," & Y & " tiene " & MapData(CurrentMap).Tile(X, Y).OBJInfo.Amount & " del Objeto " & MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex & " - " & ObjData(MapData(CurrentMap).Tile(X, Y).OBJInfo.OBJIndex).name
        End If
    Next X
Next Y
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
        If MapData(CurrentMap).Tile(X, Y).NPCIndex > 0 Then
            If MapData(CurrentMap).Tile(X, Y).NPCIndex >= 500 Then
                txtInfo.text = txtInfo.text & vbCrLf & X & "," & Y & " tiene " & NpcData(MapData(CurrentMap).Tile(X, Y).NPCIndex).name & " (Hostil)"
            Else
                txtInfo.text = txtInfo.text & vbCrLf & X & "," & Y & " tiene " & NpcData(MapData(CurrentMap).Tile(X, Y).NPCIndex).name
            End If
        End If
    Next X
Next Y
...
For Y = YMinMapSize To MapInfo(CurrentMap).Height
    For X = XMinMapSize To MapInfo(CurrentMap).Width
            If MapData(CurrentMap).Tile(X, Y).TileExit.Map > 0 Then
                txtInfo.text = txtInfo.text & vbCrLf & X & "," & Y & " nos traslada a la posición " & MapData(CurrentMap).Tile(X, Y).TileExit.X & "," & MapData(CurrentMap).Tile(X, Y).TileExit.Y & " del Mapa " & MapData(CurrentMap).Tile(X, Y).TileExit.Map
                If ((X < 20 And MapData(CurrentMap).Tile(X, Y).TileExit.X < 20) Or (X > 80 And MapData(CurrentMap).Tile(X, Y).TileExit.X > 80)) And (X <> MapData(CurrentMap).Tile(X, Y).TileExit.X) Then
                    txtInfo.text = txtInfo.text & " (X sospechoso)"
                End If
                If ((Y < 20 And MapData(CurrentMap).Tile(X, Y).TileExit.Y < 20) Or (Y > 80 And MapData(CurrentMap).Tile(X, Y).TileExit.Y > 80)) And (Y <> MapData(CurrentMap).Tile(X, Y).TileExit.Y) Then
